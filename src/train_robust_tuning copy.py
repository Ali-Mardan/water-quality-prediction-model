...
def train_robust_regularized(master_path):
    print(f"{'='*60}\nROBUST REGULARIZATION SEARCH: Full Feature Set\n{'='*60}")
    ...
    for col in df_encoded.columns:
        ...
        if df_encoded[col].dtype == 'object':
            try:
                # Try to handle dates specifically
                if 'date' in col.lower():
                    dt_series = pd.to_datetime(df_encoded[col])
                    df_encoded[col + '_year'] = dt_series.dt.year
                    df_encoded[col + '_month'] = dt_series.dt.month
                    df_encoded[col + '_day'] = dt_series.dt.day
    ...
    # Re-define available features after encoding
    available_features = [col for col in df_encoded.columns if col not in targets]
    print(f"Total features being used: {len(available_features)}")

    results_summary = []

    for target in targets:
        print(f"\n--- Tuning Target: {target} ---")
        df_target = df_encoded.dropna(subset=[target]).copy()
        
        X = df_target[available_features].copy()
        y = df_target[target].copy()
        # Find the column that represents spatial_group (it might have been encoded)
        # Actually, let's just use the original spatial_group from df
        groups = df.loc[df_target.index, 'spatial_group'].astype(str)
