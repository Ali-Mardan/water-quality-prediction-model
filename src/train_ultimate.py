...
def finalize_and_evaluate_ultimate(master_path):
    print(f"{'='*50}\nEvaluating the Ultimate 'Mega Dataset'\n{'='*50}")
    
    df = pd.read_csv(master_path)

    # Note: 'carbonate_index', 'ndci_turbidity_interaction', 'post_2008', 
    # 'urban_precip_interaction', and 'NDSI' are already inside the 
    # master_dataset_final_sota.csv that we appended the new features to.
    
    print(f"Final Feature Space Dimension: {df.shape}")
    # Display the new features to ensure they loaded
    print("\nNew Feature Summary:")
    print(df[['perc_barren', 'topographic_slope']].describe())
    ...
    for target in targets:
        ...
        # Check specific impact of new features
        print(f"\nImpact of 'perc_barren' (Mining Proxy): Rank {importances.index.get_loc('perc_barren')+1} - {importances['perc_barren']:.4f}")
        print(f"Impact of 'topographic_slope' (Hydrology): Rank {importances.index.get_loc('topographic_slope')+1} - {importances['topographic_slope']:.4f}")
    ...
    print(f"\n{'='*50}\nULTIMATE HYDRO-INFORMATICS RESULTS\n{'='*50}")
    summary_df = pd.DataFrame(results_summary)
    print(summary_df.to_string(index=False))

if __name__ == "__main__":
    finalize_and_evaluate_ultimate("master_dataset_ultimate.csv")
